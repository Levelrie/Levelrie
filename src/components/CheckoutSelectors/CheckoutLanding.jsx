    ...
    return(
        ...
        <Stack justifyItems='center' alignItems='center'>
            ...
            <Stack width={1} className='checkoutFrame' justifyItems='center' alignItems='center' mt={3} overflow='scroll'>
                {cart ? cart.map((item) => {
                return(
                    <FavoriteItemItem item={item} key={item.id} category={[item.category]} sizes={[{name: item.name, size: item.size}]} colors={[{name: item.name, color: item.color}]} inItemDetails={true}/>
                )}): 'None'}
            </Stack>
